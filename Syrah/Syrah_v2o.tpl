...
//  init_number temp_sigmaEsc;
  init_number temp_CatCV;
  init_number temp_EscCV;
...
// Extract Control data   
   ...
   !! ubAvail    = column(TempAvailability,5);
   !! phzSel     = ivector(column(TempSelectivity,2));
...
//  init_bounded_number_vector RunSize(1,NGROUPS,lbRun,ubRun,phzRun);
  ...
  number sigmaCat;
  number sigmaEsc;
  number temp_sigmaCat;
  number temp_sigmaEsc;  
  ...
  number ageCompCatchoff;
  number ageCompEscoff;
  number geneticsoff;  
...
//  ln_RunSize startRun;
//  Availability startAvail;
//  Selectivity startSel;
...
//  temp_sigmaEsc = sqrt(log(1+pow(temp_EscCV,2)));   //Assigned Log-normal CV
//  temp_sigmaCat = sqrt(log(1+pow(temp_CatCV,2)));   //Assigned Log-normal CV
    temp_sigmaEsc = temp_EscCV;
    temp_sigmaCat = temp_CatCV;
...
//Rationalize GeneticFracData - DATA STANDARDIZATION
  ...
  for(d=1;d<=NDISTRICTS;d++){
    if(GENdata(d) == 1) {
	 tempSum=0; 	
	 GeneticFracData(d)(3,NSTOCKS+2) = (GeneticFracData(d)(3,NSTOCKS+2))/sum(GeneticFracData(d)(3,NSTOCKS+2));
	 for(s=1;s<=NSTOCKS;s++) {
        tempSum += GeneticFracData(d,2+s)*log(GeneticFracData(d,2+s)+1e-5);  //Sum for multinomial		
        //tempSum
      } 	
	  }
	geneticsoff += tempSum;  
  ...
  for(d=1;d<=NDISTRICTS;d++) {
    ...
    for(ac=1;ac<=NAGECOMPS;ac++) {
      tempSum += (AgeCompCATCH(d,2+ac)*log(AgeCompCATCH(d,2+ac)+1e-05));  //Sum for multinomial	  
    }//next ac
	ageCompCatchoff += tempSum;
  }//next d  
 
   for(s=1;s<=NSTOCKS;s++) {
    tempSum=0;
   for(ac=1;ac<=NAGECOMPS;ac++) {
    tempSum += AgeCompESC(s,2+ac)*log(AgeCompESC(s,2+ac)+1e-05);  //Sum for multinomial	  
    }
	ageCompEscoff += tempSum;
	}//next s
...
//	sigmaCat = sqrt(log(1+pow(temp_CatCV,2)));   //Assigned Log-normal CV
...
// Simplified log likelihood 
	  NLLcatch += log(sigmaCat)+0.5*square(log(catchData(d))-log(catchTotal_pred(d)))/square(sigmaCat);		
...
//	sigmaEsc = sqrt(log(1+pow(temp_EscCV,2)));   //Assigned Log-normal CV
  }else {  //Log-normal MLE for sigma (2)
    for(s=1;s<=NSTOCKS;s++) {
